...
def parse_args():
    ...
    parser.add_argument(
        "--version", action="version", version="%(prog)s " + srst2_version
    )
    ...
    parser.add_argument(
        ...
        required=False,
        nargs=1,
        help="Fasta file of MLST alleles (optional)",
    )
    parser.add_argument(
        "--mlst_delimiter",
        type=str,
    ...
    parser.add_argument(
        ...
        required=False,
        nargs="+",
        help="Fasta file/s for gene databases (optional)",
    )
    parser.add_argument(
        "--no_gene_details",
        action="store_false",
        required=False,
        help="Switch OFF verbose reporting of gene typing",
    )
    parser.add_argument(
        "--gene_max_mismatch",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        required=False,
        help="Minimum %%coverage cutoff for gene reporting (default 90)",
        default=90,
    )
    parser.add_argument(
        "--max_divergence",
        type=float,
        required=False,
        help="Maximum %%divergence cutoff for gene reporting (default 10)",
        default=10,
    )
    parser.add_argument(
        "--min_depth",
        type=float,
        required=False,
        help="Minimum mean depth to flag as dubious allele call (default 5)",
        default=5,
    )
    parser.add_argument(
        "--min_edge_depth",
        type=float,
        required=False,
        help="Minimum edge depth to flag as dubious allele call (default 2)",
        default=2,
    )
    parser.add_argument(
        "--prob_err",
        type=float,
        help="Probability of sequencing error (default 0.01)",
        default=0.01,
    )
    parser.add_argument(
        "--truncation_score_tolerance",
        type=float,
        help="%% increase in score allowed to choose non-truncated allele",
        default=0.15,
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Read discarded from alignment if either of its ends has unaligned "
        + "overlap with the reference that is longer than this value (default 10)",
    )

    # Samtools parameters
    parser.add_argument(
        "--mapq", type=int, default=1, help="Samtools -q parameter (default 1)"
    )
    parser.add_argument(
        "--baseq", type=int, default=20, help="Samtools -Q parameter (default 20)"
    )
    parser.add_argument(
        "--samtools_args",
        type=str,
    ...
    # Reporting options
    parser.add_argument(
        "--output", type=str, required=True, help="Prefix for srst2 output files"
    )
    parser.add_argument(
        "--log",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="Switch ON verbose reporting of all scores",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="Use existing pileups if available, otherwise they will be generated",
    )  # to facilitate testing of rescoring from pileups
    parser.add_argument(
        "--use_existing_scores",
        action="store_true",
        required=False,
        help="Use existing scores files if available, otherwise they will be generated",
    )  # to facilitate testing of reporting from scores
    parser.add_argument(
        ...
        for line in sam:
            if not line.startswith("@"):
                ...
                if m != None:
                    ...
                    logging.info(
                        "Excluding read from SAM file due to missing NM (num mismatches) field: "
                        + fields[0]
                    )
                    num_mismatch = 0
            else:
    ...
    with open(fai_file) as fai:
        for line in fai:
            ...
            if db_type != "mlst":
                ...
                if len(allele_info) > 2:
                    ...
                    if gene_cluster in gene_cluster_symbols:
                        if gene_cluster_symbols[gene_cluster] != cluster_symbol:
                            unique_gene_symbols = (
                                False  # already seen this cluster symbol
                            )
                            logging.info(
                                "Non-unique:" + gene_cluster + ", " + cluster_symbol
                            )
                    else:
                ...
                gene_cluster = name.split(delimiter)[
                    0
...
def score_alleles(
    ...
    depth_by_gene = group_allele_dict_by_gene(
        dict(
            (allele, val)
            for (allele, val) in list(avg_depth_allele.items())
                ...
                scores_output.write(
                    "\t".join(
                        [
                            ...
                            str(this_indel),
                            str(this_missing),
                            str(this_next_to_del_depth),
                            str(mix_rate),
                            str(float(min_pval_data[0]) / min_pval_data[1]),
                            str(min_pval_data[0]),
                            str(min_pval_data[1]),
                            str(min_pval),
                        ]
...
def run_bowtie(mapping_files_pre, sample_name, fastqs, args, db_name, db_full_path):
    ...
    command += [
        ...
        "-x",
        db_full_path,  # The index to be aligned to
    ]
    if args.threads > 1:
...
def get_pileup(
    ...
    samtools_v1 = (
        check_samtools_version().split(".")[0] == "1"
    )  # Usage changed in version 1.0
...
def calculate_ST(
    ...
    if ST_db:
        ...
        try:
            ...
            print(
                "This combination of alleles was not found in the sequence type database:",
                end=" ",
            )
...
def parse_ST_database(ST_filename, gene_names_from_fai):
    ...
    print(
        "Attempting to read "
        + str(num_gene_cols_expected)
        + " loci from ST database "
        + ST_filename
    )
    with open(ST_filename) as f:
        ...
        for line in f:
            ...
            if count == 1:  # Header
                gene_names = line_split[
                    1 : min(num_gene_cols_expected + 1, len(line_split))
                ]
                for g in gene_names_from_fai:
                    if g not in gene_names:
                        print(
                            "Warning: gene "
                            + g
                            + " in database file isn't among the columns in the ST definitions: "
                            + ",".join(gene_names)
                        )
                        print(
                ...
                for g in gene_names:
                    if g not in gene_names_from_fai:
                        print(
                            "Warning: gene "
                            + g
                            + " in ST definitions file isn't among those in the database "
                ...
                if ST not in list(ST_db.values()):
                    ...
                    print(
                        "Warning: this ST is not unique in the ST definitions file: "
                        + ST
                    )
...
def get_allele_name_from_db(
    allele, run_type, args, unique_allele_symbols=False, unique_cluster_symbols=False
):
...
def group_allele_dict_by_gene(
    by_allele, run_type, args, unique_cluster_symbols=False, unique_allele_symbols=False
):
    # sort into hash for each gene locus
    by_gene = collections.defaultdict(
        dict
    ...
    for allele in by_allele:
        gene_name = get_allele_name_from_db(
            allele, run_type, args, unique_allele_symbols, unique_cluster_symbols
        )[component_ind]
    ...
    if args.input_se:
            ...
            if m == None:
                ...
                if m != None:
                    ...
                    if m != None:
                        ...
                        logging.info(
                            "Could not determine forward/reverse read status for input file "
                            + fastq
                        )
            else:
        ...
        for sample in forward_reads:
            if sample in reverse_reads:
                fileSets[sample] = [
                    forward_reads[sample],
                    reverse_reads[sample],
                ]  # store pair
                ...
                for line in f:
                    if header > 0:
                        ...
                        if "maxMAF" not in header:
                            results[
                                line.split("\t")[0]
                            ] += "\tNC"  # empty column for maxMAF
                    else:
                        header = line.rstrip()
                        results[
                            line.split("\t")[0]
                        ] = line.rstrip()  # store header line too (index "Sample")
                        if "maxMAF" not in header:
                            results[
                                line.split("\t")[0]
                            ] += "\tmaxMAF"  # add column for maxMAF
...
def run_srst2(args, fileSets, dbs, run_type):
    ...
    for fasta in dbs:
        db_reports, db_results_list = process_fasta_db(
            args, fileSets, run_type, db_reports, db_results_list, fasta
        )
...
def process_fasta_db(args, fileSets, run_type, db_reports, db_results_list, fasta):
    ...
    for sample_name in fileSets:
        ...
        try:
            ...
            gene_list, results = map_fileSet_to_db(
                args,
                sample_name,
                fastq_inputs,
                db_name,
        ...
        # log the error message, record as failed, and continue onto the next fasta db
            ...
            if run_type == "mlst":
                st_result_string = "\t".join(
                    [sample_name, "failed"] + ["-"] * (len(gene_names) + 4)
                ...
                results[sample_name][
                    "failed"
                ] = True  # so we know that we tried this strain
            ...
            if sample_name in results:
                ...
                if "failed" not in results[sample_name]:
                    ...
                    for cluster_id in gene_list:
                        ...
                        results[sample_name][
                            cluster_id
                        ] = "-f"  # record as unknown as this strain failed
            else:
...
def map_fileSet_to_db(
    ...
    if args.use_existing_scores and os.path.exists(scores_file):
        ...
        if args.use_existing_pileup and os.path.exists(pileup_file):
            ...
            (raw_bowtie_sam, bowtie_sam_mod) = modify_bowtie_sam(
                bowtie_sam,
                max_mismatch,
        ...
        scores, mix_rates = score_alleles(
            args,
            mapping_files_pre,
            hash_alignment,
            hash_max_depth,
    ...
    if run_type == "mlst" and len(allele_scores) > 0:
        ...
        if (
            uncertainty_flags != ["-"]
            and not args.save_scores
            and not os.path.exists(scores_output_file)
        ):
                ...
                scores_output.write(
                    "\t".join(
                        [
                            ...
                            str(hash_edge_depth[allele][0]),
                            str(hash_edge_depth[allele][1]),
                            str(coverage_allele[allele]),
                            str(size_allele[allele]),
                            str(mismatch_allele[allele]),
                            str(indel_allele[allele]),
                            str(missing_allele[allele]),
                            str(next_to_del_depth_allele[allele]),
    ...
    # Record gene results for later processing and optionally print detailed gene results to __fullgenes__ file
        ...
        logging.info(
            " " + str(len(allele_scores)) + " genes identified in " + sample_name
        )
    ...
    if len(mlst_results) > 0:
        ...
        for mlst_result in mlst_results:
            ...
            if "Sample" in mlst_result:
                ...
                test_string = mlst_result[
                    list(mlst_result.keys())[0]
                ]  # no header line?
            ...
            if (mlst_cols == 0) or (mlst_cols == this_mlst_cols):
                ...
                logging.info(
                    "Problem reconciling MLST data from two files, first MLST results encountered had "
                    + str(mlst_cols)
                    + " columns, this one has "
                    + str(this_mlst_cols)
                    + " columns?"
                )
                if args.mlst_db:
...
def main():
    ...
    if (len(gene_result_hashes) + len(mlst_results_hashes)) > 1:
        ...
        compile_results(
            args, mlst_results_hashes, gene_result_hashes, compiled_output_file
        )
