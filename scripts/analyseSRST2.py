...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="SRST2 results files to compile",
    )
    parser.add_argument(
        "--sample_metadata",
        nargs="+",
        type=str,
        required=False,
        help="Sample metadata to merge into table (tab delimited)",
    )
    parser.add_argument(
        "--output", type=str, required=True, help="Prefix for the output files"
    )
    parser.add_argument(
        "--log",
        action="store_true",
...
def compile_results(
    args, mlst_results, db_results, sample_metadata_hashes, compiled_output_file
):
    ...
    if len(mlst_results) > 0:
        ...
        for mlst_result in mlst_results:
            ...
            if (mlst_cols == 0) or (mlst_cols == this_mlst_cols):
                ...
                logging.info(
                    "Problem reconciling MLST data from, first MLST results encountered had "
                    + str(mlst_cols)
    ...
    for sample in sample_list:
        ...
        sample_info = (
            []
        )  # first entry is mlst string OR sample name, rest are metadata or genes
...
def main():
    ...
    sample_metadata_hashes = (
        []
    )  # dict (sample->variable->value) for each variable read from metadata file
    ...
    if (
        len(gene_result_hashes) + len(mlst_results_hashes) + len(sample_metadata_hashes)
    ) > 1:
